...
//	namespace SHMA.Enterprise.Shared
{
	public sealed class DB		
	{
		...
		//
		private static string database = System.Configuration.ConfigurationSettings.AppSettings["Database"]==null? "SQLServer":System.Configuration.ConfigurationSettings.AppSettings["Database"];

        private static readonly string EncKey = "1p8dMo5BlQ83okH3";
		[ThreadStatic]
		...
		private static System.String strConnectionstring = strEncryptedConnStr=="Y"? Encryption64.Decrypt(System.Configuration.ConfigurationSettings.AppSettings["DSN"],EncKey):System.Configuration.ConfigurationSettings.AppSettings["DSN"];		
		private static int TimeOutValue = 300;

        private static int DeadlockRetryCount = 3;
        private static bool showErrQry = System.Configuration.ConfigurationSettings.AppSettings["ShowErrQry"] == null ? false : System.Configuration.ConfigurationSettings.AppSettings["ShowErrQry"]== "Y" ? true:false;//false;
        ...
        {
            string strConnectionstring2 = strEncryptedConnStr == "Y" ? Encryption64.Decrypt(System.Configuration.ConfigurationSettings.AppSettings[DSN], EncKey) : System.Configuration.ConfigurationSettings.AppSettings[DSN];
...
//		}
        ...
        public static string GetDecryptedConnectionString(string dsnkey)
        {
            return strEncryptedConnStr == "Y" ? Encryption64.Decrypt(System.Configuration.ConfigurationSettings.AppSettings[dsnkey], EncKey) : System.Configuration.ConfigurationSettings.AppSettings[dsnkey];
        }
		...
		{
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }

            int result = 0;
            IDbCommand cmd = DB.CreateCommand(query);
			if(transaction !=null)
				cmd.Transaction = transaction;

            //result = cmd.ExecuteNonQuery();
            result = CmdExecuteDML(cmd);
			
            cmd.Dispose();
            //return result;
		}

        public static void executeDML(String query, int timeout)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
        ...
        public static int executeDML_Adhoc(String query, string DSN)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }

            int result = 0;

            IDbConnection objConnection = DB.AdhocConnection(DSN);
            if (objConnection == null)
            {
            ...
            IDbTransaction tn = objConnection.BeginTransaction();
            ...
            try
            {
                ...
                //point01 result = cmd.ExecuteNonQuery();
                result = CmdExecuteDML(cmd);
            ...
            {
                tn.Rollback();
                SHMA.Enterprise.Shared.Log.CustomExceptionLogger.Log(ex);
                string strErrMsg = "Execute Adhoc - " + ex.Message + (showErrQry ? " - " + cmd.CommandText : "");
        ...
        public static int executeDML_AdhocWT(String query, string DSN)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
        ...
        public static void executeDML(String query, ParameterCollection parameters)
		{
            ...
            int result = 0;
            IDbCommand cmd = DB.CreateCommand(query,DB.Connection);
			if (transaction != null)
				cmd.Transaction = transaction; 
        ...
        public static int ExecuteDML(String query, ParameterCollection parameters)
        {
        ...
        public static int executeDML_SP(String SPName, ParameterCollection parameters, int timeout)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(SPName); }

            int result = 0;
            IDbCommand cmd = DB.CreateSPCommand(SPName, DB.Connection);
            if (transaction != null)
                cmd.Transaction = transaction;
        ...
        public static void executeDML_Adhoc(String query, int timeout)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }

            int result = 0;
            IDbConnection objConnection = DB.AdhocConnection();
            try
            {
                IDbCommand cmd = objConnection.CreateCommand();
                cmd.CommandText = query;
        ...
        public static void executeDML_Adhoc(String query, int timeout, String DSN)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
        ...
        public static void executeDML_AdhocWOT(String query, ParameterCollection parameters, int timeout)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
        ...
        public static int executeDML_AdhocWOT(String query, ParameterCollection parameters, int timeout, string DSN)
        {
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
        ...
        public static rowset executeQuery(String query)
		{
            ...
            IDataReader dataReader = CmdExecuteQry(cmd);
		...
		public static rowset executeQuery(String query, ParameterCollection parameters)
		{
            ...
            //point01  DataSet ds = new DataSet();
			
            if (transaction != null)
				cmd.Transaction = transaction;

            //point01 IDataAdapter dataAdapter = DB.CreateDataAdapter(cmd);
            //point01 dataAdapter.Fill(ds);
            //point01 cmd.Dispose();

            //point01 DataTable dataTable = ds.Tables[0];
            DataTable dataTable = CmdExecuteAdopter(cmd); //point01 
            cmd.Dispose(); //point01 
        ...
        public static rowset executeQuery_Adhoc(String query, String DSN)
        {
            //IDbCommand cmd = DB.CreateCommand(query);
            if (logQuery) { SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(query); }
		...
		public static IDbCommand setCommandParameters(IDbCommand pstatement, ParameterCollection Parameter) 
		{
			...
			for(int counter=0; counter<Parameter.Count;counter++)
			{
				...
				Counter++;
				if(obj == null)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], objType, DBNull.Value));
				else
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], objType, Parameter.gets(Parameter.Keys[counter])));
				
/*				if  (obj is Int16)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], DbType.Int64,Parameter.gets(Parameter.Keys[counter])));
				if  (obj is Int32)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], DbType.Decimal,Parameter.gets(Parameter.Keys[counter])));
				else if(obj is Double)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], DbType.Double,Parameter.gets(Parameter.Keys[counter])));
				else if(obj is String)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], DbType.String,Parameter.gets(Parameter.Keys[counter])));
				else if(obj is DateTime)
					pstatement.Parameters.Add(DB.CreateParameter(Parameter.Keys[counter], DbType.DateTime,Parameter.gets(Parameter.Keys[counter])));
				else
        ...
        public static IDbCommand CreateSPCommand(string SPName)
        {
            IDbCommand cmd = DB.Connection.CreateCommand();
            if (transaction != null)
                cmd.Transaction = transaction;
        ...
        public static IDbCommand CreateSPCommand(string SPName, IDbConnection conn)
        {
		...
		//			return new OleDbCommand(query, DB.Connection).ExecuteReader();
		//		}
		#endregion

        private static int CmdExecuteDML(IDbCommand cmd)
        {
            int result = 0;
          int retryCount = DeadlockRetryCount; 
          bool success = false;
          ...
          {   
              ...
              {
                  if (exception.Message.ToLower().IndexOf("deadlock") < 0)      
                   {        // a sql exception that is not a deadlock         
                       string strErrMsg = "CmdExeDML - " + retryCount + exception.Message + (showErrQry ? " - " + cmd.CommandText : "");
                       throw new Exception(strErrMsg);
                   }
                  ...
                  if (retryCount == 0)
                  {
                      try
                      {
                          SHMA.Enterprise.Shared.Log.TextLogger.WriteToLogFile(exception.Message + " - " + cmd.CommandText.ToString());
                      }
                      catch { }

                      throw new Exception(exception.Message + retryCount);
                  }
              } 
          }
          return result;
        ...
        private static IDataReader CmdExecuteQry(IDbCommand cmd)
        {
            ...
            while (retryCount > 0 && !success)
            {
                ...
                catch (System.Exception exception)
                {
                    if (exception.Message.ToLower().IndexOf("deadlock") < 0)
                    {        // a sql exception that is not a deadlock         
                        string strErrMsg = "CmdExeQry - " + retryCount + exception.Message + (showErrQry ? " - " + cmd.CommandText : "");
                        throw new Exception(strErrMsg);
                    }
        ...
        private static DataTable CmdExecuteAdopter(IDbCommand cmd)
        {
            ...
            while (retryCount > 0 && !success)
            {
                try
                {
                    ...
                    IDataAdapter dataAdapter = DB.CreateDataAdapter(cmd);
                    dataAdapter.Fill(ds);
                ...
                catch (System.Exception exception)
                {
                    if (exception.Message.ToLower().IndexOf("deadlock") < 0)
                    {        // a sql exception that is not a deadlock         
                        string strErrMsg = "CmdExeAdp - " + retryCount + exception.Message + (showErrQry ? " - " + cmd.CommandText : "");
                        throw new Exception(strErrMsg);
                    }
