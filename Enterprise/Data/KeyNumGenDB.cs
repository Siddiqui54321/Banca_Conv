...
//			//myCommand.Parameters.Add(DB.CreateParameter("@PKN_VALUECOMBINATION", DbType.String, VALUE_COMBINATION));
        ...
        public static void UpdateLockNumGen(string PSE_ENTITYID, string FieldComb, string ValueComb)
        {
            //string strQuery= "SELECT PKN_STARTNO FROM PR_ED_KN_KEYNUMGEN where POR_ORGACODE = ? and  PSE_ENTITYID = ? and PKN_FIELDID = ? and PKN_AUTOMANUAL = 'A' AND ((PKN_FIELDCOMBINATION=? AND PKN_VALUECOMBINATION=?)  OR (PKN_FIELDCOMBINATION is null AND PKN_VALUECOMBINATION is null))";
            StringBuilder sbQuery = new StringBuilder(" update PR_ED_KN_KEYNUMGEN set PR_ED_KN_KEYNUMGEN.PKN_LASTNO = B.PKN_LASTNO ");
            sbQuery.Append(" FROM PR_ED_KN_KEYNUMGEN inner join PR_ED_KN_KEYNUMGEN B ");
            sbQuery.Append(" ON PR_ED_KN_KEYNUMGEN.POR_ORGACODE=b.POR_ORGACODE and ");
            sbQuery.Append(" PR_ED_KN_KEYNUMGEN.PSE_ENTITYID=b.PSE_ENTITYID and ");
            sbQuery.Append(" PR_ED_KN_KEYNUMGEN.PKL_LVLNO=b.PKL_LVLNO and ");
            sbQuery.Append(" PR_ED_KN_KEYNUMGEN.PKN_SRNO=B.PKN_SRNO AND ");
            sbQuery.Append(" PR_ED_KN_KEYNUMGEN.PKN_FIELDID=B.PKN_FIELDID ");
            ...
            IDbCommand myCommand = DB.CreateCommand(sbQuery.ToString());
            myCommand.Parameters.Add(DB.CreateParameter("@PSE_ENTITYID", DbType.String, PSE_ENTITYID));
            myCommand.Parameters.Add(DB.CreateParameter("@PKN_FIELDCOMBINATION", DbType.String, FieldComb));
            myCommand.Parameters.Add(DB.CreateParameter("@PKN_VALUECOMBINATION", DbType.String, ValueComb));
            myCommand.Transaction = DB.Transaction;
