...
namespace SHMA.Enterprise.Presentation
{
	public class TwoStepController: PageController
	{
		...
		protected override void  Page_Load(object sender, System.EventArgs e )
		{
            try
            {
                if(false)//if (this.ToString().ToUpper().IndexOf("LOGIN") < 0 && isSessionExpired())
                {
                ...
                else
                {
                    ...
                    //Rehan// Code to handle duboule posting agianst Page refresh
                    /*
                    if (Master != null)
                    {
                        System.Web.UI.WebControls.TextBox ctl;
                        ctl = (System.Web.UI.WebControls.TextBox)Master.FindControl("myForm").FindControl("RequestGUID");
                        if (ctl != null)
                        {
                            if (IsPostBack && ctl.Text != Convert.ToString(Session["REFRESH_CHECK_GUID"]))
                            {
                                IsRefresh = true;
                            }
                            //Convert.ToInt16(ViewState["REFRESH_CHECK_GUID"] == null ? 0 : ViewState["REFRESH_CHECK_GUID"]) + 1;//
                            Session["REFRESH_CHECK_GUID"] = System.Guid.NewGuid().ToString();
                            ctl.Text = Session["REFRESH_CHECK_GUID"].ToString();
                        }
                    }
                    */
                    //Rehan//--------------------------------------------------------------------
        ...
        {
            bool rtnvalue = false;
            if (SessionObject.Get("s_SUS_USERCODE") == null)
            {
                rtnvalue = true;
            }
            return rtnvalue;
