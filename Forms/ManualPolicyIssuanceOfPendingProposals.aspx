		...
		<form id="Form1" method="post" runat="server">
			...
			<table cellpadding="0" cellspacing="0" width="100%" border="0">
				<tr class="form_heading">
					<td height="20" colSpan="4" valign="middle" align="center">Pending Proposal List-1
								...
								<asp:TemplateColumn HeaderText="Status">
									<ItemTemplate>
										<asp:DropDownList ID="ddlStatus" Runat="server" Width="4.0pc" CssClass="text_font">
                                         ...
                                         <asp:ListItem Value=".">.</asp:ListItem>
    <asp:ListItem Value="Y">Ok</asp:ListItem>
    <asp:ListItem Value="Y">Proceed to CBC</asp:ListItem>
    <asp:ListItem Value="Y">Referred to GM</asp:ListItem>
    <asp:ListItem Value="Y">Approved</asp:ListItem>
    <asp:ListItem Value="T">Discrepant</asp:ListItem>
...
<div id="myModal" class="modal">
 ...
 <script type="text/javascript">
    ...
    var userType = '<%= Session["UserType"] %>';
         var dropdowns = grid.querySelectorAll("select[id*='ddlStatus']");
         ...
         dropdowns.forEach(function (ddl) {
             ...
             // Always show the blank/default option
             showOption(ddl, ".");

             // ---- Logic for specific combinations ----
             if (bankCode === "F" && userType === "W") {
                 showOption(ddl, "Y", "Proceed to CBC");
                 showOption(ddl, "T", "Discrepant");
                 showOption(ddl, "R", "Rejected");
                 showOption(ddl, "D", "Declined"); // ✅ fixed match
             }
             else if (bankCode === "F" && userType === "M") {
                 showOption(ddl, "Y", "Referred to GM");
                 showOption(ddl, "R", "Rejected");
             }
             else if (bankCode === "F" && userType === "H") {
                 showOption(ddl, "Y", "Approved");
                 showOption(ddl, "R", "Rejected");
             }
             else if ((bankCode === "9" && userType === "M") || (bankCode === "9" && userType === "H")) {
                 showOption(ddl, "Y", "Ok");
                 showOption(ddl, "N", "Not Ok");
             }

             // Default select
             if (!ddl.value || ddl.value === "") ddl.value = ".";
         });

         // ✅ Helper function to unhide specific option
         function showOption(ddl, value, textMatch) {
             for (var k = 0; k < ddl.options.length; k++) {
                 var opt = ddl.options[k];
                 var cleanText = opt.text.replace(/["';]/g, "").trim();
                 if (
                     opt.value === value &&
                     (!textMatch || cleanText.toLowerCase() === textMatch.toLowerCase())
                 ) {
