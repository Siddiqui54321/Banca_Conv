...
namespace ace
{
	...
	public class clsIlasReport
	{
		...
		public clsIlasReport(string Proposal, string ReportType)
		{
		...
		private string startProcess()
		{
			...
			string vField  = clsIlasConstant.REPORT;
			int vfLevel = 0;
			try
			{
				//************** Field combination **************//
				...
				else
				{
					...
					{
						throw new Exception("REPORT is not defined in Product Field Setup (LPVF_VALIDFIELDS)");
					}
				}
		...
		private void setMoreFieldsInCollection()
		{
			try
			{
				vfCollection.setValue("REPORT_TYPE", this.reportType);
		...
		private string getAccumulatedError(string product, int serialNo)
		{
			...
			while(rsValidationFields.next())
			{
				...
				vField = rsValidationFields.getString("PVD_DATATYPE");
...
//					}
					...
					else
					{
						...
						processedError = " " + htmlFormatStart + vfName + " Should be " + htmlFormatEnd + vfOperator + rangeFrom + " ";
					}
				}
				else
				{
