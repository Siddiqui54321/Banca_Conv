...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_ILUS_ET_UM_USERMANAGMENT : SHMA.Enterprise.Presentation.TwoStepController
	{
		...
		{
			Sql = "select a.use_userid, a.user_email   from USE_USERMASTER a  where a.use_userid = '" + txtUSE_USERID.Text.ToString() + "'";

			// txtEmail.Text = GetData(").Rows[0]["user_email"].ToString();

			dtUserEmail = GetData(Sql);
			if (dtUserEmail.Rows.Count > 0)

			{
				txtEmail.Text = dtUserEmail.Rows[0]["user_email"].ToString();
			}
		...
		void UpdateEmail()
		{
			...
			Sql = "update USE_USERMASTER B set B.user_email='" + txtEmail.Text.ToString() + "'  where B.use_userid = '" + txtUSE_USERID.Text.ToString() + "'";
