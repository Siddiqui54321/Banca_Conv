...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LCCN_COLUMN : SHMA.Enterprise.Presentation.TwoStepController{
		...
		protected System.Web.UI.HtmlControls.HtmlGenericControl EntryTableDiv;
	...
	#region Major methods of First Step		
		...
		sealed protected override DataHolder GetInputData(DataHolder dataHolder)
		{
			GetSessionValues();
			CheckKeyLevel();
			//recordCount = LCCN_COLUMNDB.RecordCount;
			return   dataHolder;      
		}
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			DataTable table = new DataTable("LCCN_COLUMN");
			IDataReader LCCN_COLUMNReader= LCCN_COLUMNDB.GetLCCN_COLUMN_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			recordSelected = IsRecordSelected();
			//recordSelected = IsRecordSelected(LCCN_COLUMNReader, LCCN_COLUMN.PrimaryKeys, "LCCN_COLUMN");
			//LCCN_COLUMNReader= LCCN_COLUMNDB.GetLCCN_COLUMN_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
				recordCount = Utilities.Reader2Table(LCCN_COLUMNReader, table, PAGE_SIZE, LCCN_COLUMN.PrimaryKeys, out pageNumber);
			}
			else
			{
				recordCount = Utilities.Reader2Table(LCCN_COLUMNReader, table, PAGE_SIZE, pageNumber);
			}
			LCCN_COLUMNReader.Close();
			BindLister(table);							
			...
			txtCCN_COLUMNID.Attributes.Add("onblur","setColumnPrefix(this);");
	...
	#region Major methods of the final step
		protected override void ValidateRequest() {
			...
			foreach (string key in LCCN_COLUMN.PrimaryKeys){
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			case (EnumControlArgs.Save):
				...
				dataHolder = new LCCN_COLUMNDB(dataHolder).FindByPK(txtCCN_COLUMNID.Text);
				columnNameValue.Add("CCN_COLUMNID",txtCCN_COLUMNID.Text.Trim()==""?null:txtCCN_COLUMNID.Text);
columnNameValue.Add("CCN_DESCRIPTION",txtCCN_DESCRIPTION.Text.Trim()==""?null:txtCCN_DESCRIPTION.Text);
columnNameValue.Add("CCN_SQLSTATEMENT",txtCCN_SQLSTATEMENT.Text.Trim()==""?null:txtCCN_SQLSTATEMENT.Text);
columnNameValue.Add("CCN_DATATYPE",ddlCCN_DATATYPE.SelectedValue.Trim()==""?null:ddlCCN_DATATYPE.SelectedValue);
								
				security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), LCCN_COLUMN.PrimaryKeys, columnNameValue, "LCCN_COLUMN");
				if (security.SaveAllowed){
					
					new LCCN_COLUMN(dataHolder).Add(columnNameValue,getAllFields(),"LCCN_COLUMN",null);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()),LCCN_COLUMN.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_INSERT, "LCCN_COLUMN");
					_lastEvent.Text = "Save"; 					
...
columnNameValue.Add("CCN_DATATYPE",ddlCCN_DATATYPE.SelectedValue.Trim()==""?null:ddlCCN_DATATYPE.SelectedValue);
				...
				if (security.UpdateAllowed){
					...
					new LCCN_COLUMN(dataHolder).Update(Utilities.File2EntityID(this.ToString()),columnNameValue);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), LCCN_COLUMN.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_UPDATE, "LCCN_COLUMN");
					recordSelected = true;
...
columnNameValue.Add("CCN_DATATYPE",ddlCCN_DATATYPE.SelectedValue.Trim()==""?null:ddlCCN_DATATYPE.SelectedValue);
				...
				new LCCN_COLUMN(dataHolder).Delete(columnNameValue);

				dataHolder.Update(DB.Transaction);
				
				auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), LCCN_COLUMN.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_DELETE, "LCCN_COLUMN");
...
columnNameValue.Add("CCN_DATATYPE",ddlCCN_DATATYPE.SelectedValue.Trim()==""?null:ddlCCN_DATATYPE.SelectedValue);
				...
				if (_CustomArgName.Value == "ProcessName"){
					...
					if (security.ProcessAllowed(processName)){
						...
						if (type != null){
							...
							proccessCommand.setPrimaryKeys(LCCN_COLUMN.PrimaryKeys);
							proccessCommand.setTableName("LCCN_COLUMN");
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			LCCN_COLUMNDB LCCN_COLUMNDB_obj = new LCCN_COLUMNDB(dataHolder);		
			IDataReader LCCN_COLUMNReader;
			DataTable table = new DataTable("LCCN_COLUMN") ;

			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				DataRow row = LCCN_COLUMNDB_obj.FindByPK(CCN_COLUMNID)["LCCN_COLUMN"].Rows[0];
				ShowData(row);
			}		
			else{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LCCN_COLUMNReader = LCCN_COLUMNDB.GetLCCN_COLUMN_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LCCN_COLUMNReader = LCCN_COLUMNDB.GetLCCN_COLUMN_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LCCN_COLUMNReader, table, PAGE_SIZE, pageNumber);
				LCCN_COLUMNReader.Close();
				...
				{
					ShowData(dataHolder["LCCN_COLUMN"].Rows[0]);
				}		
			}
			...
			if (_lastEvent.Text == EnumControlArgs.View.ToString()){
				SHSM_SecurityPermission security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), LCCN_COLUMN.PrimaryKeys, columnNameValue, "LCCN_COLUMN");
				if (!security.UpdateAllowed)
	...
	#region Events
		...
		private void lister_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e) {
			...
			if (e.CommandName == "Edit") {								
				if (e.Item.ItemType==ListItemType.Item){									
					CCN_COLUMNID=((LinkButton)e.Item.FindControl("linkCCN_COLUMNID1")).Text;

				}
				else if (e.Item.ItemType==ListItemType.AlternatingItem){
					CCN_COLUMNID=((LinkButton)e.Item.FindControl("linkCCN_COLUMNID2")).Text;
		...
		private void lister_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e) 
		{
			...
			LinkButton linkCCN_COLUMNID = new LinkButton();
			if (e.Item.ItemType==ListItemType.Item){
				linkCCN_COLUMNID = (LinkButton)e.Item.FindControl("linkCCN_COLUMNID1");
			}
			else if (e.Item.ItemType==ListItemType.AlternatingItem){
				linkCCN_COLUMNID=(LinkButton)e.Item.FindControl("linkCCN_COLUMNID2");	
			}			
			tRow.Attributes.Add("onclick", linkCCN_COLUMNID.ClientID + ".click();" );
		}
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			...
			{
				SessionObject.Set("CCN_COLUMNID",txtCCN_COLUMNID.Text);
				//SessionObject.Set("CCN_DESCRIPTION",txtCCN_DESCRIPTION.Text);
				//SessionObject.Set("CCN_SQLSTATEMENT",txtCCN_SQLSTATEMENT.Text);
				//SessionObject.Set("CCN_DATATYPE",ddlCCN_DATATYPE.SelectedValue);
			}
		}	
		...
		void RefreshDataFields()
		{
			...
			txtCCN_COLUMNID.Enabled = true;
txtCCN_COLUMNID.Text="";
txtCCN_DESCRIPTION.Text="";
txtCCN_SQLSTATEMENT.Text="";
ddlCCN_DATATYPE.ClearSelection();
		...
		{
			RefreshDataFields();
			txtCCN_COLUMNID.Text=objRow["CCN_COLUMNID"].ToString();
txtCCN_COLUMNID.Enabled=false;
txtCCN_DESCRIPTION.Text=objRow["CCN_DESCRIPTION"].ToString();
txtCCN_SQLSTATEMENT.Text=objRow["CCN_SQLSTATEMENT"].ToString();
ddlCCN_DATATYPE.ClearSelection();
ListItem item3=ddlCCN_DATATYPE.Items.FindByValue(objRow["CCN_DATATYPE"].ToString());
if (item3!= null){
item3.Selected=true;
}
		...
		bool IsRecordSelected(){
			bool selected = true;
			foreach (string pk in LCCN_COLUMN.PrimaryKeys){
		...
		private void FindAndSelectCurrentRecord(System.Web.UI.WebControls.RepeaterItemEventArgs e)
		{
			...
			if (recordFound){
				...
				((HtmlTableRow)item.FindControl("ListerRow")).Attributes["class"] = "ListerSelItem";
				DataRow selectedRow = new LCCN_COLUMNDB(dataHolder).FindByPK(CCN_COLUMNID)["LCCN_COLUMN"].Rows[0];
