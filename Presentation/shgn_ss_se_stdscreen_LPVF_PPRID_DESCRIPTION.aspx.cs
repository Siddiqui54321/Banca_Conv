...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LPVF_PPRID_DESCRIPTION : SHMA.Enterprise.Presentation.TwoStepController{
	...
	#region Major methods of First Step		
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			IDataReader LPPR_PRODUCTReader0 = LPPR_PRODUCTDB.GetDDL_LPVF_PPRID_DESCRIPTION_PPR_PRODCD_RO();;
...
LPPR_PRODUCTReader0.Close();
			...
			IDataReader LPVF_VALIDFIELDSReader= LPVF_VALIDFIELDSDB.GetLPVF_PPRID_DESCRIPTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			...
			//LPVF_VALIDFIELDSReader= LPVF_VALIDFIELDSDB.GetLPVF_PPRID_DESCRIPTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
			...
			HeaderScript.Text = EnvHelper.Parse("function setDefaultValues(){getField(\"PVF_CODE\").value = SV(\"VFS_CODE\");}") ;
	...
	#region Major methods of the final step
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			shgn.SHGNCommand entityClass=new ace.MandatoryRiderProdEntityClass();
			...
			case (EnumControlArgs.Save):
				...
				dataHolder = new LPVF_VALIDFIELDSDB(dataHolder).FindByPK(ddlPPR_PRODCD.SelectedValue,txtPVF_CODE.Text);
...
columnNameValue.Add("PVF_CODE",txtPVF_CODE.Text.Trim()==""?null:txtPVF_CODE.Text);
				...
				if (security.SaveAllowed){
					...
					new LPVF_VALIDFIELDS(dataHolder).Add(columnNameValue,getAllFields(),"LPVF_PPRID_DESCRIPTION",null);
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPVF_VALIDFIELDSReader = LPVF_VALIDFIELDSDB.GetLPVF_PPRID_DESCRIPTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPVF_VALIDFIELDSReader = LPVF_VALIDFIELDSDB.GetLPVF_PPRID_DESCRIPTION_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPVF_VALIDFIELDSReader, table, PAGE_SIZE, pageNumber);
				LPVF_VALIDFIELDSReader.Close();
...
PVF_CODE=((Label)e.Item.FindControl("lblPVF_CODE2")).Text;
		...
		SessionObject.Set("PVF_CODE",txtPVF_CODE.Text);
...
txtPVF_CODE.Text=objRow["PVF_CODE"].ToString();
txtPVF_CODE.Enabled=false;
