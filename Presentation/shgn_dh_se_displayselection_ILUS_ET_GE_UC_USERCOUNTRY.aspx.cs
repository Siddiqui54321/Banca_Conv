...
namespace SHAB.Presentation{
	public partial class shgn_dh_se_displayselection_ILUS_ET_GE_UC_USERCOUNTRY : SHMA.Enterprise.Presentation.PageController
	{
		protected SHMA.Enterprise.Presentation.WebControls.DropDownList USE_USERID ;
	...
	#region Major methods of the final step      		
		...
		sealed protected override void DataBind(DataHolder dataHolder) {
			...
			IDataReader drUSE_USERID  = USE_USERMASTERDB.GetDDL_ILUS_ET_GE_UC_USERCOUNTRY_USE_USERID_RO();
USE_USERID .DataSource = drUSE_USERID ;
USE_USERID .DataBind();
drUSE_USERID .Close();
		...
		void AcquireSessionValues(){
			if(SessionObject.Get("USE_USERID ")!= null){
USE_USERID .ClearSelection();
ListItem item0 = USE_USERID .Items.FindByValue(SessionObject.Get("USE_USERID ").ToString());
if (item0 != null){item0.Selected=true;}}
