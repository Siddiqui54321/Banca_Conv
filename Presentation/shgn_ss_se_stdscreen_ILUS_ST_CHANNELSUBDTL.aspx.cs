...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_ILUS_ST_CHANNELSUBDTL : SHMA.Enterprise.Presentation.TwoStepController{
...
protected System.Web.UI.WebControls.Literal ltlCCD_CODE;
	...
	#region Major methods of First Step		
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			DataTable table = new DataTable("CCS_CHANLSUBDETL");
			IDataReader CCS_CHANLSUBDETLReader= CCS_CHANLSUBDETLDB.GetILUS_ST_CHANNELSUBDTL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			recordSelected = IsRecordSelected();
			if (recordSelected)
			{
				recordCount = Utilities.Reader2Table(CCS_CHANLSUBDETLReader, table, PAGE_SIZE, CCS_CHANLSUBDETL.PrimaryKeys, out pageNumber);
			}
			else
			{
				recordCount = Utilities.Reader2Table(CCS_CHANLSUBDETLReader, table, PAGE_SIZE, pageNumber);
			}
			CCS_CHANLSUBDETLReader.Close();
			BindLister(table);							
	...
	#region Major methods of the final step
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			case (EnumControlArgs.Save):
				...
				dataHolder = new CCS_CHANLSUBDETLDB(dataHolder).FindByPK(txtCCD_CODE.Text,txtCCH_CODE.Text,txtCCS_CODE.Text);
...
columnNameValue.Add("CCS_DESCR",txtCCS_DESCR.Text.Trim()==""?null:txtCCS_DESCR.Text);
				...
				if (security.SaveAllowed){
					...
					new CCS_CHANLSUBDETL(dataHolder).Add(columnNameValue,getAllFields(),"ILUS_ST_CHANNELSUBDTL",null);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()),CCS_CHANLSUBDETL.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_INSERT, "CCS_CHANLSUBDETL");
					_lastEvent.Text = "Save"; 					
...
columnNameValue.Add("CCS_DESCR",txtCCS_DESCR.Text.Trim()==""?null:txtCCS_DESCR.Text);
				...
				if (security.UpdateAllowed){
					...
					new CCS_CHANLSUBDETL(dataHolder).Update(Utilities.File2EntityID(this.ToString()),columnNameValue);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), CCS_CHANLSUBDETL.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_UPDATE, "CCS_CHANLSUBDETL");
					recordSelected = true;
...
columnNameValue.Add("CCS_DESCR",txtCCS_DESCR.Text.Trim()==""?null:txtCCS_DESCR.Text);
				...
				new CCS_CHANLSUBDETL(dataHolder).Delete(columnNameValue);
...
columnNameValue.Add("CCS_DESCR",txtCCS_DESCR.Text.Trim()==""?null:txtCCS_DESCR.Text);
				...
				if (_CustomArgName.Value == "ProcessName"){
					...
					if (security.ProcessAllowed(processName)){
						...
						if (type != null){
							...
							proccessCommand.setPrimaryKeys(CCS_CHANLSUBDETL.PrimaryKeys);
							proccessCommand.setTableName("CCS_CHANLSUBDETL");
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			CCS_CHANLSUBDETLDB CCS_CHANLSUBDETLDB_obj = new CCS_CHANLSUBDETLDB(dataHolder);		
			IDataReader CCS_CHANLSUBDETLReader;
			DataTable table = new DataTable("CCS_CHANLSUBDETL") ;

			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				DataRow row = CCS_CHANLSUBDETLDB_obj.FindByPK(CCD_CODE,CCH_CODE,CCS_CODE)["CCS_CHANLSUBDETL"].Rows[0];
				ShowData(row);
			}		
			else{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					CCS_CHANLSUBDETLReader = CCS_CHANLSUBDETLDB.GetILUS_ST_CHANNELSUBDTL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					CCS_CHANLSUBDETLReader = CCS_CHANLSUBDETLDB.GetILUS_ST_CHANNELSUBDTL_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(CCS_CHANLSUBDETLReader, table, PAGE_SIZE, pageNumber);
				CCS_CHANLSUBDETLReader.Close();
				...
				{
					ShowData(dataHolder["CCS_CHANLSUBDETL"].Rows[0]);
				}		
			}
			...
			if (_lastEvent.Text == EnumControlArgs.View.ToString()){
				SHSM_SecurityPermission security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), CCS_CHANLSUBDETL.PrimaryKeys, columnNameValue, "CCS_CHANLSUBDETL");
				if (!security.UpdateAllowed)
		...
		private void lister_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e) {
			...
			if (e.CommandName == "Edit") {								
				if (e.Item.ItemType==ListItemType.Item){									
					CCS_CODE=((LinkButton)e.Item.FindControl("linkCCS_CODE1")).Text;
CCH_CODE=((Label)e.Item.FindControl("lblCCH_CODE1")).Text;
CCD_CODE=((Label)e.Item.FindControl("lblCCD_CODE1")).Text;

				}
				else if (e.Item.ItemType==ListItemType.AlternatingItem){
					CCS_CODE=((LinkButton)e.Item.FindControl("linkCCS_CODE2")).Text;
...
CCD_CODE=((Label)e.Item.FindControl("lblCCD_CODE2")).Text;
		...
		private void lister_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e) 
		{
			...
			LinkButton linkCCS_CODE = new LinkButton();
			if (e.Item.ItemType==ListItemType.Item){
				linkCCS_CODE = (LinkButton)e.Item.FindControl("linkCCS_CODE1");
			}
			else if (e.Item.ItemType==ListItemType.AlternatingItem){
				linkCCS_CODE=(LinkButton)e.Item.FindControl("linkCCS_CODE2");	
			}			
			tRow.Attributes.Add("onclick", linkCCS_CODE.ClientID + ".click();" );
		}
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			//base.OnUnload(e);
			if (SetFieldsInSession()){
				SessionObject.Set("CCS_CODE",txtCCS_CODE.Text);
		SessionObject.Set("CCS_DESCR",txtCCS_DESCR.Text);
		...
		{
			if (SessionObject.Get("CCH_CODE")==null  || SessionObject.GetString("CCH_CODE")== ""  || SessionObject.Get("CCD_CODE")==null  || SessionObject.GetString("CCD_CODE")== "" ){	
				DisableForm();
...
txtCCS_DESCR.Text=objRow["CCS_DESCR"].ToString();
		...
		bool IsRecordSelected(){
			bool selected = true;
			foreach (string pk in CCS_CHANLSUBDETL.PrimaryKeys){
...
CCD_CODE=((Label)e.Item.FindControl("lblCCD_CODE2")).Text;
				...
				((HtmlTableRow)item.FindControl("ListerRow")).Attributes["class"] = "ListerSelItem";
				DataRow selectedRow = new CCS_CHANLSUBDETLDB(dataHolder).FindByPK(CCD_CODE,CCH_CODE,CCS_CODE)["CCS_CHANLSUBDETL"].Rows[0];
