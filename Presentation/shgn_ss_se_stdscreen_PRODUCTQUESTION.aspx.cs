...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_PRODUCTQUESTION : SHMA.Enterprise.Presentation.TwoStepController{
...
protected System.Web.UI.WebControls.Literal ltlPPR_PRODCD;
	...
	#region Major methods of First Step		
		...
		sealed protected override DataHolder GetInputData(DataHolder dataHolder)
		{
			GetSessionValues();
			CheckKeyLevel();
			//recordCount = LPQN_QUESTIONNAIREDB.RecordCount;
			return   dataHolder;      
		}
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			IDataReader LCQN_QUESTIONNAIREReader0 = LCQN_QUESTIONNAIREDB.GetDDL_PRODUCTQUESTION_CQN_CODE_RO();;
ddlCQN_CODE.DataSource = LCQN_QUESTIONNAIREReader0 ;
ddlCQN_CODE.DataBind();
LCQN_QUESTIONNAIREReader0.Close();
			
			_lastEvent.Text = "New";		

			DataTable table = new DataTable("LPQN_QUESTIONNAIRE");
			IDataReader LPQN_QUESTIONNAIREReader= LPQN_QUESTIONNAIREDB.GetPRODUCTQUESTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			recordSelected = IsRecordSelected();
			//recordSelected = IsRecordSelected(LPQN_QUESTIONNAIREReader, LPQN_QUESTIONNAIRE.PrimaryKeys, "LPQN_QUESTIONNAIRE");
			//LPQN_QUESTIONNAIREReader= LPQN_QUESTIONNAIREDB.GetPRODUCTQUESTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
				recordCount = Utilities.Reader2Table(LPQN_QUESTIONNAIREReader, table, PAGE_SIZE, LPQN_QUESTIONNAIRE.PrimaryKeys, out pageNumber);
			}
			else
			{
				recordCount = Utilities.Reader2Table(LPQN_QUESTIONNAIREReader, table, PAGE_SIZE, pageNumber);
			}
			LPQN_QUESTIONNAIREReader.Close();
			BindLister(table);							
		

			HeaderScript.Text = EnvHelper.Parse("") ;
			FooterScript.Text = EnvHelper.Parse("var product = SV(\"PPR_PRODCD1\"); function setDefaultValues(){getField(\"PPR_PRODCD\").value=product;}") ;
	...
	#region Major methods of the final step
		protected override void ValidateRequest() {
			...
			foreach (string key in LPQN_QUESTIONNAIRE.PrimaryKeys){
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			case (EnumControlArgs.Save):
				...
				dataHolder = new LPQN_QUESTIONNAIREDB(dataHolder).FindByPK(ddlCQN_CODE.SelectedValue,txtPPR_PRODCD.Text);
				columnNameValue.Add("PPR_PRODCD",txtPPR_PRODCD.Text.Trim()==""?null:txtPPR_PRODCD.Text);
				columnNameValue.Add("CQN_CODE",ddlCQN_CODE.SelectedValue.Trim()==""?null:ddlCQN_CODE.SelectedValue);
								
				security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), LPQN_QUESTIONNAIRE.PrimaryKeys, columnNameValue, "LPQN_QUESTIONNAIRE");
				if (security.SaveAllowed){
					
					new LPQN_QUESTIONNAIRE(dataHolder).Add(columnNameValue,getAllFields(),"PRODUCTQUESTION",null);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()),LPQN_QUESTIONNAIRE.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_INSERT, "LPQN_QUESTIONNAIRE");
					_lastEvent.Text = "Save"; 					
...
columnNameValue.Add("CQN_CODE",ddlCQN_CODE.SelectedValue.Trim()==""?null:ddlCQN_CODE.SelectedValue);
				...
				if (security.UpdateAllowed){
					...
					new LPQN_QUESTIONNAIRE(dataHolder).Update(Utilities.File2EntityID(this.ToString()),columnNameValue);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), LPQN_QUESTIONNAIRE.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_UPDATE, "LPQN_QUESTIONNAIRE");
					recordSelected = true;
...
columnNameValue.Add("CQN_CODE",ddlCQN_CODE.SelectedValue.Trim()==""?null:ddlCQN_CODE.SelectedValue);
				...
				new LPQN_QUESTIONNAIRE(dataHolder).Delete(columnNameValue);

				dataHolder.Update(DB.Transaction);
				
				auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), LPQN_QUESTIONNAIRE.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_DELETE, "LPQN_QUESTIONNAIRE");
...
columnNameValue.Add("CQN_CODE",ddlCQN_CODE.SelectedValue.Trim()==""?null:ddlCQN_CODE.SelectedValue);
				...
				if (_CustomArgName.Value == "ProcessName"){
					...
					if (security.ProcessAllowed(processName)){
						...
						if (type != null){
							...
							proccessCommand.setPrimaryKeys(LPQN_QUESTIONNAIRE.PrimaryKeys);
							proccessCommand.setTableName("LPQN_QUESTIONNAIRE");
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			LPQN_QUESTIONNAIREDB LPQN_QUESTIONNAIREDB_obj = new LPQN_QUESTIONNAIREDB(dataHolder);		
			IDataReader LPQN_QUESTIONNAIREReader;
			DataTable table = new DataTable("LPQN_QUESTIONNAIRE") ;

			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				DataRow row = LPQN_QUESTIONNAIREDB_obj.FindByPK(CQN_CODE,PPR_PRODCD)["LPQN_QUESTIONNAIRE"].Rows[0];
				ShowData(row);
			}		
			else{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPQN_QUESTIONNAIREReader = LPQN_QUESTIONNAIREDB.GetPRODUCTQUESTION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPQN_QUESTIONNAIREReader = LPQN_QUESTIONNAIREDB.GetPRODUCTQUESTION_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPQN_QUESTIONNAIREReader, table, PAGE_SIZE, pageNumber);
				LPQN_QUESTIONNAIREReader.Close();
				...
				{
					ShowData(dataHolder["LPQN_QUESTIONNAIRE"].Rows[0]);
				}		
			}
			...
			if (_lastEvent.Text == EnumControlArgs.View.ToString()){
				SHSM_SecurityPermission security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), LPQN_QUESTIONNAIRE.PrimaryKeys, columnNameValue, "LPQN_QUESTIONNAIRE");
				if (!security.UpdateAllowed)
...
PPR_PRODCD=((Label)e.Item.FindControl("lblPPR_PRODCD2")).Text;
		...
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			...
			if (SetFieldsInSession()){
				...
				SessionObject.Set("CQN_CODE",ddlCQN_CODE.SelectedValue);
...
//			}
		...
		void RefreshDataFields()
		{
			...
			txtPPR_PRODCD.Text="";
ddlCQN_CODE.Enabled = true;
ddlCQN_CODE.ClearSelection();
...
ddlCQN_CODE.ClearSelection();
ListItem item1=ddlCQN_CODE.Items.FindByValue(objRow["CQN_CODE"].ToString());
if (item1!= null){
item1.Selected=true;
}ddlCQN_CODE.Enabled=false;
		...
		bool IsRecordSelected(){
			bool selected = true;
			foreach (string pk in LPQN_QUESTIONNAIRE.PrimaryKeys){
...
PPR_PRODCD=((Label)e.Item.FindControl("lblPPR_PRODCD2")).Text;
				...
				((HtmlTableRow)item.FindControl("ListerRow")).Attributes["class"] = "ListerSelItem";
				DataRow selectedRow = new LPQN_QUESTIONNAIREDB(dataHolder).FindByPK(CQN_CODE,PPR_PRODCD)["LPQN_QUESTIONNAIRE"].Rows[0];
