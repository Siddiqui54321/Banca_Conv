...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LPVL_VALIDATION_DECISION : SHMA.Enterprise.Presentation.TwoStepController
	{
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			IDataReader LCDC_DECISIONReader0 = LCDC_DECISIONDB.GetDDL_LPVL_VALIDATION_DECISION_PVL_VALIDRANGE_RO();;
			ddlPVL_VALIDRANGE.DataSource = LCDC_DECISIONReader0 ;
			ddlPVL_VALIDRANGE.DataBind();
			LCDC_DECISIONReader0.Close();
			
			_lastEvent.Text = "New";		

			DataTable table = new DataTable("LPVL_VALIDATION");
			IDataReader LPVL_VALIDATIONReader= LPVL_VALIDATIONDB.GetLPVL_VALIDATION_DECISION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			...
			//LPVL_VALIDATIONReader= LPVL_VALIDATIONDB.GetLPVL_VALIDATION_DECISION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder)
		{
			...
			switch ((EnumControlArgs)ControlArgs[0])
			{
				case (EnumControlArgs.Save):
					...
					columnNameValue.Add("PVL_VALIDRANGE",ddlPVL_VALIDRANGE.SelectedValue.Trim()==""?null:ddlPVL_VALIDRANGE.SelectedValue);
					...
					if (security.SaveAllowed)
					{
						...
						new LPVL_VALIDATION(dataHolder).Add(columnNameValue,getAllFields(),"LPVL_VALIDATION_DECISION",null);
		...
		{			
			...
			else
			{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPVL_VALIDATIONReader = LPVL_VALIDATIONDB.GetLPVL_VALIDATION_DECISION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPVL_VALIDATIONReader = LPVL_VALIDATIONDB.GetLPVL_VALIDATION_DECISION_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPVL_VALIDATIONReader, table, PAGE_SIZE, pageNumber);
				LPVL_VALIDATIONReader.Close();
		...
		private void lister_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e) 
		{
			...
			{								
				...
				{									
					PVL_LEVEL=double.Parse(((LinkButton)e.Item.FindControl("linkPVL_LEVEL1")).Text);
				...
				{
					PVL_LEVEL=double.Parse(((LinkButton)e.Item.FindControl("linkPVL_LEVEL2")).Text);
		...
		private void lister_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e) 
		{
			...
			LinkButton linkPVL_LEVEL = new LinkButton();
			if (e.Item.ItemType==ListItemType.Item)
			{
				linkPVL_LEVEL = (LinkButton)e.Item.FindControl("linkPVL_LEVEL1");
			}
			else if (e.Item.ItemType==ListItemType.AlternatingItem)
			{
				linkPVL_LEVEL=(LinkButton)e.Item.FindControl("linkPVL_LEVEL2");	
			}			
			tRow.Attributes.Add("onclick", linkPVL_LEVEL.ClientID + ".click();" );
		}
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			...
			if (SetFieldsInSession())
			{
				...
				//SessionObject.Set("PVL_VALIDRANGE",ddlPVL_VALIDRANGE.SelectedValue);
		...
		void RefreshDataFields()
		{
			...
			txtPVL_LEVEL.Text="0";
			ddlPVL_VALIDRANGE.ClearSelection();
		...
		protected void ShowData(DataRow objRow)
		{
			...
			ddlPVL_VALIDRANGE.ClearSelection();
			ListItem item3=ddlPVL_VALIDRANGE.Items.FindByValue(objRow["PVL_VALIDRANGE"].ToString());
			if (item3!= null)
			{
				item3.Selected=true;
