...
function parentChanged(parentObj, cboID) {
    ...
    cboID = GetTableField(parentObj, cboID);
	...
	if(valueColumns!=null && valueColumns.length>2){
		for(cntr=1;cntr<valueColumns.length;cntr++){
			document.getElementById(GetTableField(comboText, valueColumns[cntr])).value="";
		}
	}
...
{	
	...
	var txtFW=trim(txt.getAttribute("fieldsWidth"));
	var txtFT=trim(txt.getAttribute("fieldsTitle"));
	var txtFY=trim(txt.getAttribute("fieldsType"));
	if(txtFW==null || txtFW=="")
	{
		alert("Popup LOV width is not Properly defined!");	
		return;
	}
	if(txtFT==null || txtFT=="")
	{
		alert("Popup LOV title is not Properly defined!");	
		return;
	}
	if(txtFY==null || txtFY=="")
	{
		alert("Popup LOV fields type is not Properly defined!");	
		return;
	}
	...
	{
		field = getTabularFieldByPeer(txt, strSplit[0]);
	...
	var retValue=showModalDialog(Url, 'window', 'dialogWidth:480px;dialogHeight:310px;center:yes;status:off;help:off');	
	if(retValue!=null)
	{
	    var retVal = retValue.split('~');
		var mapStrCol=Url.split('^');
		var mapCol=mapStrCol[5].split('-');
		var mapColLength = retVal.length;
		
		if(mapColLength==3)
		{
			document.getElementById(mapCol[0]).value=retVal[0];
			document.getElementById(mapCol[1]).value=retVal[1];
			if(mapCol[2]!=null && mapCol[2].length>0) document.getElementById(mapCol[2]).value=retVal[2];
		}
		else if(mapColLength==2)
		{
			document.getElementById(mapCol[0]).value=retVal[0];
			if(mapCol[1]!=null && mapCol[1].length>0) document.getElementById(mapCol[1]).value=retVal[1];
		}
        ...
        ////Added by Syed Kamran 22092014
        var newMethod = new Function(document.getElementById(mapCol[0]).getAttribute("JavascriptOnChangeFunction"));
        if(newMethod!=null)
          newMethod();
...
function show(cboID, pageNo, criterion){
	...
	//alert("comboFrame===="+comboFrame);
	comboText = document.getElementById(cboID);
	//alert("comboText===="+comboText);
	comboDiv = document.getElementById('div'+cboID);
	//alert("comboDiv===="+comboDiv);
...
function putValue(strCode,strDesc, cboID, selectedIndex){	
	...
	if (comboText.value!=strCode || valueField.indexOf(",")>1){
		...
		if (comboText.getAttribute("onchange")!=null){
			//AD;18/08/2009;START;
			//comboText.onchange();
			var newMethod = new Function(comboText.getAttribute("JavascriptOnChangeFunction"));
			newMethod();
	...
	if(arrValues.length >1){
		for(cntr=0;cntr < arrValues.length;cntr++){
			...
			if(lov.arrCodes==null){
				...
				document.getElementById(GetTableField(lov, arrValues[cntr])).value = arrCodes[ cntr + (seletecdIndex * arrValues.length)];
				}
			else
				document.getElementById(GetTableField(lov, arrValues[cntr])).value = arrCodes[ cntr + (seletecdIndex * arrValues.length)];
		}
...
function putComboDescription(cboID, descID)
{
    ...
    seperatedValues = parseSendValueInfo(queryExtraVALUE, cboID);
...
function putOtherDescription(field, val){
	...
	if(vals[0]!=null && vals[1]!=null){
		...
		if(arrayValues !=null && arrayValues.length>1)
		{
			...
			{
				valField = getTabularFieldByPeer(field, arrayValues[cntr]);
				...
				if(vals[cntr]!=null)
					getTabularFieldByPeer(field, arrayValues[cntr]).value = vals[cntr];
				else
					getTabularFieldByPeer(field, arrayValues[cntr]).value = "";
			}
		}
		...
		//		if(descs[cntr]!=null){
		//			getTabularFieldByPeer(field, arrayDescs[cntr]).value = descs[cntr];
		//			}
		//		else
		//			getTabularFieldByPeer(field, arrayDescs[cntr]).value =  "";
		//			
		//	}
		//}
	return true;
...
function GetSearchQuery(cboID, criterion){
	...
	if (textFields!=null && textFields.length>0){
		query="select  " + txt.textFields;
		if(txt.descriptionColumn!=null && descriptionColumn.length>0 && descriptionColumn.indexOf(",") > 1)
		{
	...
	query += parseExtraInfo(  getHandledExtraInfo(queryExtraInfo), cboID );		
...
function GetQuery(cboID){
	...
	if (whereColumn!=null && whereColumn.length>0){
		...
		for(cnt=0; cnt<whereColumns.length; cnt++){					
			...
			if	(whereValues[cnt].indexOf("_cl")>=0) {
				whereValues[cnt] = whereValues[cnt].replace("'","").replace("'","");
				//alert("whereValues[cnt]====>"+whereValues[cnt]);
				//alert("cboID====>"+cboID);
				whereID = GetMyRowField(cboID, whereValues[cnt].replace('_cl',''));
				//alert("whereID........"+whereID);
				//alert("whereID===>>>"+document.getElementById(whereID).value);
				query += "'" +  document.getElementById(whereID).value + "'";
			}
			else{
				query+= whereValues[cnt];
			}			
...
*/
function parseSendValueInfo(qei, cboID) {
    temp = qei;
    start = -1;
    end = -1;
    fieldName = "";
    ...
    while (true) {
        ...
        if (start > -1) {
            ...
            fID = GetTableFieldIDByQEI(cboID, fieldName); //W:GetTableField(getField(cboID), fieldName);
            var filed = document.getElementById(fID);
            val = filed.value; 		//W:getField(filed).value;
...
function getHandledExtraInfo(queryExtraInfo){		
	...
	if (queryExtraInfo!=null && queryExtraInfo.length>0){	
...
function parseExtraInfo(qei, cboID){
	temp = qei;
	start  =  -1;
	end  =  -1;
	fieldName ="";			
...
function getWhereValues(whereColumn, whereValue, whereOperator, valueField, cboID) {
    ...
    if (whereValue != null && whereValue.length > 0) {
        ...
        // whereColumns = whereColumn.split(',');
        whereValues = whereValue.split(',');
        //alert('whereValues : ' + whereValues);
        //whereOperators = whereOperator.split(',');
        valueColumns = valueField.split(',');
        //alert('valueColumns : ' + valueColumns);
        for (cnt = 0; cnt < whereValues.length; cnt++)
        {
            ...
            if (whereValues[cnt].indexOf("_cl") >= 0) {
                ...
                //alert('query_whereValues : ' + query_whereValues);
                //alert(GetMyRowField(cboID, whereValues[cnt].replace("_cl", "")));
            }
            else {
                //query += whereValues[cnt];
...
function getComboLeft(comboText){
	return Combo_Up_findPosX(comboText) -10;
}
function getComboTop(comboText){
	return Combo_Up_findPosY(comboText) -9;
}

function GetMyRowField( siblingID, myID){	
	
	ind = siblingID.replace('__','--').indexOf('_')+1;	
	compID = siblingID.substring(ind, siblingID.length);

	return siblingID.replace(compID, myID);	
	...
	if(parentObj.indexOf('New')>0 && myID.indexOf('New')<0 ){
		return myID.replace('txt','txtNew');
	}
	alert("myID"+myID);
	return myID;
	AD;18/08/2009;END*/
...
function GetTableField(parentObj, myID){
    name = parentObj.name;
	qName = name.substring(0, name.lastIndexOf('$') + 1);
	var myName = ""
	if(name.indexOf('New')>0){
		myName = qName + "txtNew" + myID.replace('txtNew','').replace('txt','');
		//W:return qName + "txtNew" + myID.replace('txtNew','').replace('txt','');
		//else if(getField(qName + "ddlNew" + myID) !=null)
		//	return qName + "ddlNew" + myID;
	}
	else //if(getField(qName + "txt" + myID) !=null)
		myName = qName + "txt" + myID.replace('txt','').replace('txtNew','');
		//W:return qName + "txt" + myID.replace('txt','').replace('txtNew','');
		//return qName + "txt" + myID;
		//else if(getField(qName + "ddl" + myID) !=null)
		//return qName + "ddl" + myID;
	return myName;
...
function GetTableFieldIDByQEI(qeiID, myID){
	var name = qeiID;
	var qName = name.substring(0,name.lastIndexOf('_txt')+4);
	if(name.indexOf('New')>0){
		return qName + "New" + myID.replace('txtNew','').replace('txt','');
	}
	else 
		return qName + myID.replace('txt','').replace('txtNew','');
}