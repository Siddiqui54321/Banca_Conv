...
function _RSExecuteSynchronously(rspage,method,parameters,context) {	
	...
	var data;
	var isIE8 = checkIEVersion();
	//alert(url.length);
	if(isIE8 || url.length <= 2035)
	{
		//alert('>=8');
		data = msrsDoSynchronousGet(url);
	}
	else
	{
		//alert('6,7')
		url = getUrlforPOST(rspage,method,parameters);
		data = msrsDoSynchronousPost(url[0], url[1]);
	}	
...
function RSExecute( rspage, method ) {
  ...
  if (synchronous) {
	...
	var rrr=_RSExecuteSynchronously(rspage,method,parameters,context);
	return rrr;	
  }
...
function evalRequest(request)
{
	...
	if ((start_index = data.indexOf(start_key)) != -1)
	{
		...
		else if (metatag.indexOf('EVAL_OBJECT') != -1)
		{
		    ...
		    //alert('request.return_value : ' + request.return_value);
...
function getUrlforPOST( rsPage, func, parms ){
  // build URL to call
  var result = [];
  var par = "";
  var URL = rsPage;
  ...
  if (func != null){
	...
	if (parms != null || parms.length == 0){
		// assume parms is array of strings
		for( var i=0; i < parms.length; i++ ){
			par += "p" + i + "=" + escape(parms[i]+'') + "";
		}
		par += "&pcount=" + parms.length;
    } else {
		URL += "&pcount=0";
    } // parms
  } // func  
...
function msrsDoSynchronousPost(url, params) {
  var remote = msrsGetSynchronousHttpObject();
  remote.open("POST",url,false);
  
//Send the proper header information along with the request
remote.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
remote.setRequestHeader("Content-length", params.length);
...
{
   var rv = -1; // indicating the use of another browser.
...
{
	var ver = getInternetExplorerVersion(); //alert('ver: '+ver);
	if ( ver> -1 ) //indicating the browser is IE
	{
		if ( ver>= 8.0 )
		{
			return true;
