...
*************************************************************************
	Modified by :	Ahmed
	Date		:	3-3-05
	For			:	UI problem with DropDown List
	Increments	:	CreateLayer() , hideLayer()	
************************************************************************/	
...
function CalendarPopup_Up_HideNonCurrentCalendar(divName, myName){
	if(CalendarPopup_curMonthYear != '') {
		...
		hideLayer(CalendarPopup_curMonthYear+"_Layer");
	...
	if(CalendarPopup_curCalendar != '') {
		...
		hideLayer(CalendarPopup_curCalendar+"_Layer");
...
function CalendarPopup_Up_DisplayCalendarByDate(tbName, lblName, divName, myName, funcName, myFuncName, stringDate, wdStyle, weStyle, omStyle, sdStyle, mhStyle, dhStyle, cdStyle, tdStyle, gttStyle, holStyle, formatNum, monthnames, daynames, fdweek, sunNum, satNum, enableHide, includeYears, lBound, uBound, pad, postbackFunc, showClear, clearText, showGoToToday, goToTodayText, arrowUrl, customFunc, calWidth, visibleKey, nullText, dateArray, nextMonthImgUrl, prevMonthImgUrl, nextYearImgUrl, prevYearImgUrl) {
	...
	CreateLayer(divName+"_Layer", document.getElementById(divName).offsetTop, document.getElementById(divName).offsetLeft, document.getElementById(divName).offsetWidth, document.getElementById(divName).offsetHeight)
}
...
function CreateLayer(overID, nTop, nLeft, nWidth, nHeight)
{
 if (!document.getElementById(overID))
 {
  ...
  //elem.insertAdjacentHTML("beforeEnd", "<iframe id='" + overID + "' src='about:blank' style='position:absolute;left:0px;top:0x;z-index:499;display:none' scrolling='no' frameborder='0'></iframe>");
  elem.appendChild( document.createElement( 'div' ) ).innerHTML= "<iframe id='" + overID + "' src='about:blank' style='position:absolute;left:0px;top:0x;z-index:499;display:none' scrolling='no' frameborder='0'></iframe>";
 ...
 if(document.getElementById(overID))
 {
  ...
  overs.left = nLeft;
  overs.width = nWidth;
  overs.height = nHeight;
//  overs.filter = 'progidXImageTransform.Microsoft.Alpha(style=0,opacity=0)';
  overs.display = 'block';
 }
}

function hideLayer(overID) 
{
 if (document.getElementById(overID))
 {
  var overs = document.getElementById(overID).style;
