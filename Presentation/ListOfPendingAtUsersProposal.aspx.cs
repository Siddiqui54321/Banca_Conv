...
namespace SHAB.Presentation
{
	...
	/// </summary>
    public partial class ListOfPendingAtUsersProposal : SHMA.Enterprise.Presentation.TwoStepController
	{
		#region " First Step "		 
        ...
        protected void btn_getData_Click(object sender, EventArgs e)
        {
            try
            {
				...
				string query = "Select listagg(ccs_code,',') WITHIN GROUP (order by ccs_code) ChannelMapping  From luch_userchannel where use_userid like '" + user + "'";
			    ...
			    query = "select np1_proposal,np1_propdate,nph_fullname,nph_idno,nad_mobile,np1_selected,np1_checked,cm_commentby,cm_comments,\n"+
						"case when np1_selected='P' and np1_checked='Y' then 'Proposal is pending at RBH'\n" +
						"            when np1_selected='R' and np1_checked='N' then 'Proposal is pending at CBC Approved for Collection'\n" +
						"           when np1_selected='C' and np1_checked='N' then 'Proposal is pending at BM Approved for Collection'\n" +
						"           when np1_selected='P' and np1_checked='N' then 'Proposal is pending at BM Compliance OK'\n" +
						"              end screens\n" +
						" From vw_cpuinquiry \n" +
									" where np1_channeldetail = '" + bankCode + "' \n"+
									" and np1_channelsdetail in ("+mapping+")";
                if (ddl_SelectOption.SelectedValue=="1")
                {
					query += "and np1_selected in ('P','C')\n"+
								"and np1_checked='N'";
				}
				if (ddl_SelectOption.SelectedValue == "2")
				{
					query += "and np1_selected ='P'\n" +
								"and np1_checked='Y'";
				}
				if (ddl_SelectOption.SelectedValue == "3")
				{
					query += "and np1_selected ='R'\n" +
								"and np1_checked='N'";
				}
		...
		private void ExportGridToExcel()
		{
			...
			Response.Charset = "";
			string FileName = "Pending_Proposals_At_Users" + DateTime.Now + ".xls";
