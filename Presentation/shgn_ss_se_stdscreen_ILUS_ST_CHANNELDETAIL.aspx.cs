...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_ILUS_ST_CHANNELDETAIL : SHMA.Enterprise.Presentation.TwoStepController{
...
protected System.Web.UI.WebControls.Literal ltlCCH_CODE;
	...
	#region Major methods of First Step		
		...
		sealed protected override DataHolder GetInputData(DataHolder dataHolder)
		{
			GetSessionValues();
			CheckKeyLevel();
			//recordCount = CCD_CHANNELDETAILDB.RecordCount;
			return   dataHolder;      
		}
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			DataTable table = new DataTable("CCD_CHANNELDETAIL");
			IDataReader CCD_CHANNELDETAILReader= CCD_CHANNELDETAILDB.GetILUS_ST_CHANNELDETAIL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			recordSelected = IsRecordSelected();
			if (recordSelected)
			{
				recordCount = Utilities.Reader2Table(CCD_CHANNELDETAILReader, table, PAGE_SIZE, CCD_CHANNELDETAIL.PrimaryKeys, out pageNumber);
			}
			else
			{
				recordCount = Utilities.Reader2Table(CCD_CHANNELDETAILReader, table, PAGE_SIZE, pageNumber);
			}
			CCD_CHANNELDETAILReader.Close();
			BindLister(table);							
	...
	#region Major methods of the final step
		protected override void ValidateRequest() {
			...
			foreach (string key in CCD_CHANNELDETAIL.PrimaryKeys){
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			case (EnumControlArgs.Save):
				...
				dataHolder = new CCD_CHANNELDETAILDB(dataHolder).FindByPK(txtCCD_CODE.Text,txtCCH_CODE.Text);
...
columnNameValue.Add("CCD_DESCR",txtCCD_DESCR.Text.Trim()==""?null:txtCCD_DESCR.Text);
								
				security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), CCD_CHANNELDETAIL.PrimaryKeys, columnNameValue, "CCD_CHANNELDETAIL");
				if (security.SaveAllowed){
					
					new CCD_CHANNELDETAIL(dataHolder).Add(columnNameValue,getAllFields(),"ILUS_ST_CHANNELDETAIL",null);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()),CCD_CHANNELDETAIL.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_INSERT, "CCD_CHANNELDETAIL");
					_lastEvent.Text = "Save"; 					
...
columnNameValue.Add("CCD_DESCR",txtCCD_DESCR.Text.Trim()==""?null:txtCCD_DESCR.Text);
				...
				if (security.UpdateAllowed){
					...
					new CCD_CHANNELDETAIL(dataHolder).Update(Utilities.File2EntityID(this.ToString()),columnNameValue);

					dataHolder.Update(DB.Transaction);
					
					auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), CCD_CHANNELDETAIL.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_UPDATE, "CCD_CHANNELDETAIL");
					recordSelected = true;
...
columnNameValue.Add("CCD_DESCR",txtCCD_DESCR.Text.Trim()==""?null:txtCCD_DESCR.Text);
				...
				new CCD_CHANNELDETAIL(dataHolder).Delete(columnNameValue);

				dataHolder.Update(DB.Transaction);
				
				auditTrail.fssaveAuditLog(Utilities.File2EntityID(this.ToString()), CCD_CHANNELDETAIL.PrimaryKeys, columnNameValue, SHSM_AuditTrail.DML_OPERATION_DELETE, "CCD_CHANNELDETAIL");
...
columnNameValue.Add("CCD_DESCR",txtCCD_DESCR.Text.Trim()==""?null:txtCCD_DESCR.Text);
				...
				if (_CustomArgName.Value == "ProcessName"){
					...
					if (security.ProcessAllowed(processName)){
						...
						if (type != null){
							...
							proccessCommand.setPrimaryKeys(CCD_CHANNELDETAIL.PrimaryKeys);
							proccessCommand.setTableName("CCD_CHANNELDETAIL");
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			CCD_CHANNELDETAILDB CCD_CHANNELDETAILDB_obj = new CCD_CHANNELDETAILDB(dataHolder);		
			IDataReader CCD_CHANNELDETAILReader;
			DataTable table = new DataTable("CCD_CHANNELDETAIL") ;

			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				DataRow row = CCD_CHANNELDETAILDB_obj.FindByPK(CCD_CODE,CCH_CODE)["CCD_CHANNELDETAIL"].Rows[0];
				ShowData(row);
			}		
			else{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					CCD_CHANNELDETAILReader = CCD_CHANNELDETAILDB.GetILUS_ST_CHANNELDETAIL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					CCD_CHANNELDETAILReader = CCD_CHANNELDETAILDB.GetILUS_ST_CHANNELDETAIL_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(CCD_CHANNELDETAILReader, table, PAGE_SIZE, pageNumber);
				CCD_CHANNELDETAILReader.Close();
				...
				{
					ShowData(dataHolder["CCD_CHANNELDETAIL"].Rows[0]);
				}		
			}
			...
			if (_lastEvent.Text == EnumControlArgs.View.ToString()){
				SHSM_SecurityPermission security = new SHSM_SecurityPermission( Utilities.File2EntityID(this.ToString()), CCD_CHANNELDETAIL.PrimaryKeys, columnNameValue, "CCD_CHANNELDETAIL");
				if (!security.UpdateAllowed)
		...
		private void lister_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e) {
			...
			if (e.CommandName == "Edit") {								
				if (e.Item.ItemType==ListItemType.Item){									
					CCD_CODE=((LinkButton)e.Item.FindControl("linkCCD_CODE1")).Text;
CCH_CODE=((Label)e.Item.FindControl("lblCCH_CODE1")).Text;

				}
				else if (e.Item.ItemType==ListItemType.AlternatingItem){
					CCD_CODE=((LinkButton)e.Item.FindControl("linkCCD_CODE2")).Text;
CCH_CODE=((Label)e.Item.FindControl("lblCCH_CODE2")).Text;
		...
		private void lister_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e) 
		{
			...
			LinkButton linkCCD_CODE = new LinkButton();
			if (e.Item.ItemType==ListItemType.Item){
				linkCCD_CODE = (LinkButton)e.Item.FindControl("linkCCD_CODE1");
			}
			else if (e.Item.ItemType==ListItemType.AlternatingItem){
				linkCCD_CODE=(LinkButton)e.Item.FindControl("linkCCD_CODE2");	
			}			
			tRow.Attributes.Add("onclick", linkCCD_CODE.ClientID + ".click();" );
		}
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			//base.OnUnload(e);
			if (SetFieldsInSession()){
				SessionObject.Set("CCD_CODE",txtCCD_CODE.Text);
		SessionObject.Set("CCD_DESCR",txtCCD_DESCR.Text);
		...
		{
			if (SessionObject.Get("CCH_CODE")==null  || SessionObject.GetString("CCH_CODE")== "" ){	
				DisableForm();
...
txtCCD_DESCR.Text=objRow["CCD_DESCR"].ToString();
		...
		bool IsRecordSelected(){
			bool selected = true;
			foreach (string pk in CCD_CHANNELDETAIL.PrimaryKeys){
...
CCH_CODE=((Label)e.Item.FindControl("lblCCH_CODE2")).Text;
				...
				((HtmlTableRow)item.FindControl("ListerRow")).Attributes["class"] = "ListerSelItem";
				DataRow selectedRow = new CCD_CHANNELDETAILDB(dataHolder).FindByPK(CCD_CODE,CCH_CODE)["CCD_CHANNELDETAIL"].Rows[0];
