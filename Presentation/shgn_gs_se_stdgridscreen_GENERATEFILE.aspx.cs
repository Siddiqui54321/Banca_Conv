...
namespace SHAB.Presentation
{
	...
	public partial class shgn_gs_se_stdgridscreen_GENERATEFILE : SHMA.Enterprise.Presentation.TwoStepController{
...
#region Major methods of First Step
	...
	sealed protected override DataHolder GetInputData(DataHolder dataHolder)
	{
		GetSessionValues();
		//CheckKeyLevel();
		//recordCount = DUMMYDB.RecordCount;
		return   dataHolder;         
	...
	{	
		...
		DateTime currentDate = Convert.ToDateTime(SessionObject.GetString("s_CURR_SYSDATE"));
		currentDate = currentDate.AddDays(1);
		
		HeaderScript.Text = EnvHelper.Parse("") ;
		FooterScript.Text = EnvHelper.Parse("getField(\"DATETO\").value='"+currentDate.ToString("dd/MM/yyyy")+"';  getField(\"DATEFROM\").value=SV(\"s_CURR_SYSDATE\");") ;			
	}
#endregion
...
#region Major methods of the final step
				...
				if (_CustomArgName.Value == "ProcessName"){
					...
					//if (security.ProcessAllowed(processName)){
						...
						if (type != null){
							...
							//proccessCommand.setPrimaryKeys(DUMMY.PrimaryKeys);
							//proccessCommand.setTableName("DUMMY");
	...
	sealed protected override void DataBind(DataHolder dataHolder){		
		...
		/*
		 * HeaderScript.Text = EnvHelper.Parse("");
		 * FooterScript.Text = EnvHelper.Parse("getField(\"DATEFROM\").value=SV(\"s_CURR_SYSDATE\");  getField(\"DATETO\").value=SV(\"s_CURR_SYSDATE\");");				
		*/
		DateTime currentDate = Convert.ToDateTime(SessionObject.GetString("s_CURR_SYSDATE"));
		currentDate = currentDate.AddDays(1);
...
#region Events
	protected void _CustomEvent_ServerClick(object sender, System.EventArgs e) {
		...
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			...
			/*SessionObject.Set("DATEFROM",txtDATEFROM.Text);
		SessionObject.Set("DATETO",txtDATETO.Text);
		*/ 
		}
		/**
					...
					switch (c.GetType().ToString()) {
						...
						case "SHMA.Enterprise.Presentation.WebControls.DatePopUp":
							if (c.ID.IndexOf("txt")==0)
								...
								_fieldName = c.ID;
							if (!columnNameValue.Contains(_fieldName)){
								allFields.add(_fieldName, ((SHMA.Enterprise.Presentation.WebControls.DatePopUp)c).Text);
							}
							break;
