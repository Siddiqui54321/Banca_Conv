...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LPVD_VALIDATIONDETAIL : SHMA.Enterprise.Presentation.TwoStepController{
...
protected System.Web.UI.WebControls.Literal ltlPVL_LEVEL;
	...
	#region Major methods of First Step		
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			IDataReader LPVD_VALIDATIONDETAILReader= LPVD_VALIDATIONDETAILDB.GetLPVD_VALIDATIONDETAIL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			...
			//LPVD_VALIDATIONDETAILReader= LPVD_VALIDATIONDETAILDB.GetLPVD_VALIDATIONDETAIL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
...
columnNameValue.Add("PVD_VALIDTO",txtPVD_VALIDTO.Text.Trim()==""?null:txtPVD_VALIDTO.Text);
				...
				if (security.SaveAllowed){
					...
					new LPVD_VALIDATIONDETAIL(dataHolder).Add(columnNameValue,getAllFields(),"LPVD_VALIDATIONDETAIL",null);
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPVD_VALIDATIONDETAILReader = LPVD_VALIDATIONDETAILDB.GetLPVD_VALIDATIONDETAIL_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPVD_VALIDATIONDETAILReader = LPVD_VALIDATIONDETAILDB.GetLPVD_VALIDATIONDETAIL_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPVD_VALIDATIONDETAILReader, table, PAGE_SIZE, pageNumber);
				LPVD_VALIDATIONDETAILReader.Close();
...
PVD_LEVEL=double.Parse(((Label)e.Item.FindControl("lblPVD_LEVEL2")).Text);
		...
		{
			if (SessionObject.Get("PPR_PRODCD_S")==null  || SessionObject.GetString("PPR_PRODCD_S")== ""  || SessionObject.Get("PVL_VALIDATIONFOR")==null  || SessionObject.GetString("PVL_VALIDATIONFOR")== ""  || SessionObject.Get("PVL_LEVEL")==null  || SessionObject.GetString("PVL_LEVEL")== "" ){	
				DisableForm();
