...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LPVF_VALIDFIELDS_VALIDATION : SHMA.Enterprise.Presentation.TwoStepController
	{
		...
		{
			chkCallValidation.Checked=false;
	
			IDataReader LVFS_FIELDSETUPReader0 = LVFS_FIELDSETUPDB.GetDDL_LPVF_VALIDFIELDS_VALIDATION_PVF_CODE_RO();;
			ddlPVF_CODE.DataSource = LVFS_FIELDSETUPReader0 ;
			ddlPVF_CODE.DataBind();
			LVFS_FIELDSETUPReader0.Close();
			
			_lastEvent.Text = "New";		

			DataTable table = new DataTable("LPVF_VALIDFIELDS");
			IDataReader LPVF_VALIDFIELDSReader= LPVF_VALIDFIELDSDB.GetLPVF_VALIDFIELDS_VALIDATION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
		...
		public void saveUpdateCallValidation(String ProductID,String callValidationFor)
		{
			if (chkCallValidation.Checked)
			{
				if(!(LPVL_VALIDATIONDB.IsCallValidationExist(ProductID,callValidationFor)))
				{
					LPVL_VALIDATIONDB.AddCallValidation(ProductID,callValidationFor);
				}
			}
			else
			{
				LPVL_VALIDATIONDB.RemoveCallValidation(ProductID,callValidationFor);			
			}
		}
			...
			switch ((EnumControlArgs)ControlArgs[0])
			{
				case (EnumControlArgs.Save):
					...
					if (security.SaveAllowed)
					{
						...
						new LPVF_VALIDFIELDS(dataHolder).Add(columnNameValue,getAllFields(),"LPVF_VALIDFIELDS_VALIDATION",null);
						...
						saveUpdateCallValidation(txtPPR_PRODCD.Text,ddlPVF_CODE.SelectedValue);
				...
				case (EnumControlArgs.Delete):
					...
					if (security.DeleteAllowed)
					{
						...
						LPVL_VALIDATIONDB.RemoveCallValidation(txtPPR_PRODCD.Text,ddlPVF_CODE.SelectedValue);
		...
		{			
			...
			else
			{
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPVF_VALIDFIELDSReader = LPVF_VALIDFIELDSDB.GetLPVF_VALIDFIELDS_VALIDATION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPVF_VALIDFIELDSReader = LPVF_VALIDFIELDSDB.GetLPVF_VALIDFIELDS_VALIDATION_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPVF_VALIDFIELDSReader, table, PAGE_SIZE, pageNumber);
				LPVF_VALIDFIELDSReader.Close();
		...
		private void lister_ItemCommand(object source, System.Web.UI.WebControls.RepeaterCommandEventArgs e) 
		{
			...
			{								
				...
				{									
					PVF_CODE=((LinkButton)e.Item.FindControl("linkPVF_CODE1")).Text;
					PPR_PRODCD=((Label)e.Item.FindControl("lblPPR_PRODCD1")).Text;

				}
				else if (e.Item.ItemType==ListItemType.AlternatingItem)
				{
					PVF_CODE=((LinkButton)e.Item.FindControl("linkPVF_CODE2")).Text;
		...
		private void lister_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e) 
		{
			...
			LinkButton linkPVF_CODE = new LinkButton();
			if (e.Item.ItemType==ListItemType.Item)
			{
				linkPVF_CODE = (LinkButton)e.Item.FindControl("linkPVF_CODE1");
			}
			else if (e.Item.ItemType==ListItemType.AlternatingItem)
			{
				linkPVF_CODE=(LinkButton)e.Item.FindControl("linkPVF_CODE2");	
			}			
			tRow.Attributes.Add("onclick", linkPVF_CODE.ClientID + ".click();" );
		}
		protected void Page_Unload(object sender, System.EventArgs e)
		{
			...
			{
				SessionObject.Set("PPR_PRODCD_S",txtPPR_PRODCD.Text);
				SessionObject.Set("PVF_CODE",ddlPVF_CODE.SelectedValue);
				//SessionObject.Set("PVF_TYPE",txtPVF_TYPE.Text);
				//SessionObject.Set("PVF_FIELDCOMB",txtPVF_FIELDCOMB.Text);
			}
		}	
		...
		protected void ShowData(DataRow objRow)
		{
			...
			ddlPVF_CODE.ClearSelection();
			ListItem item2=ddlPVF_CODE.Items.FindByValue(objRow["PVF_CODE"].ToString());
			if (item2!= null)
			{
				item2.Selected=true;
			}ddlPVF_CODE.Enabled=false;
			txtPVF_FIELDCOMB.Text=objRow["PVF_FIELDCOMB"].ToString();
			...
			chkCallValidation.Checked=LPVL_VALIDATIONDB.IsCallValidationExist(txtPPR_PRODCD.Text,ddlPVF_CODE.SelectedValue);
		}
