...
namespace SHAB.Presentation
{
	...
	public partial class shgn_ss_se_stdscreen_LPVL_RIDER_INFORMATION : SHMA.Enterprise.Presentation.TwoStepController{
...
protected System.Web.UI.WebControls.Literal ltlPPR_PRODCD;
	...
	#region Major methods of First Step		
		...
		sealed protected override void BindInputData(DataHolder dataHolder)
		{
			...
			IDataReader LPVL_VALIDATIONReader= LPVL_VALIDATIONDB.GetLPVL_RIDER_INFORMATION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			...
			//LPVL_VALIDATIONReader= LPVL_VALIDATIONDB.GetLPVL_RIDER_INFORMATION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);
			if (recordSelected)
			{
			...
			HeaderScript.Text = EnvHelper.Parse("function setDefaultValues(){getField(\"PVL_VALIDATIONFOR\").value = SV(\"VFS_CODE\"); getField(\"PVL_LEVEL\").value = \"0\";}") ;
	...
	#region Major methods of the final step
		...
		sealed protected override void ApplyDomainLogic(DataHolder dataHolder){
			...
			shgn.SHGNCommand entityClass=new ace.MandatoryRiderEntityClass();
			SessionObject.Set("PVL_VALIDRANGE", txtPVL_VALIDRANGE.Text);
			...
			case (EnumControlArgs.Save):
				...
				columnNameValue.Add("PVL_VALIDRANGE",txtPVL_VALIDRANGE.Text.Trim()==""?null:txtPVL_VALIDRANGE.Text);
				...
				if (security.SaveAllowed){
					...
					new LPVL_VALIDATION(dataHolder).Add(columnNameValue,getAllFields(),"LPVL_RIDER_INFORMATION",null);
		...
		sealed protected override void DataBind(DataHolder dataHolder){			
			...
			if ((EnumControlArgs)ControlArgs[0] == EnumControlArgs.Edit){
				...
				if (ViewState["filterVal"]==null || ViewState["filterVal"].ToString().Trim()=="%")
					LPVL_VALIDATIONReader = LPVL_VALIDATIONDB.GetLPVL_RIDER_INFORMATION_lister_RO(pageNumber * PAGE_SIZE, PAGE_SIZE);//get_Orders_Data_RO();				
				else
					LPVL_VALIDATIONReader = LPVL_VALIDATIONDB.GetLPVL_RIDER_INFORMATION_lister_filter_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());//get_Orders_Data_RO(ViewState["filterCol"].ToString(), ViewState["filterVal"].ToString());
				recordCount = Utilities.Reader2Table(LPVL_VALIDATIONReader, table, PAGE_SIZE, pageNumber);
				LPVL_VALIDATIONReader.Close();
...
txtPVL_VALIDRANGE.Text=objRow["PVL_VALIDRANGE"].ToString();
