...
{
	public partial class shgn_ts_se_tblscreen_ILUS_ET_UC_USERCOUNTRY : SHMA.Enterprise.Presentation.TwoStepController
	{
	...
	#region Major methods of First Step
		...
		{	
			ClearSession();
			GetSessionValues();
			dataHolder = new LUCN_USERCOUNTRYDB(dataHolder).GetILUS_ET_UC_USERCOUNTRY_Data();
			return   dataHolder;         
		}
...
columnNameValue.Add("UCN_DEFAULT",((DropDownList)footerItem.Cells[0].FindControl("ddlNewUCN_DEFAULT")).SelectedValue.Trim()==""?null:((DropDownList)footerItem.Cells[0].FindControl("ddlNewUCN_DEFAULT")).SelectedValue);
					   ...
					   LUCN_USERCOUNTRY_obj.Add(columnNameValue,GetAnItemData(footerItem),"ILUS_ET_UC_USERCOUNTRY",null);
...
IDataReader drCCN_CTRYCD = LCCN_COUNTRYDB.GetDDL_ILUS_ET_UC_USERCOUNTRY_CCN_CTRYCD_RO();
